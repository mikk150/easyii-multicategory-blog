<?php
use traffic\easyii\lender\models\LoanType;
...
<?php if ($settings['categoryThumb']) : ?>
    <?php if ($model->image) : ?>
        ...
        <a href="<?= Url::to(['/admin/'.$this->context->module->id.'/a/clear-image', 'id' => $model->primaryKey]) ?>" class="text-danger confirm-delete" title="<?= Yii::t('easyii', 'Clear image')?>"><?= Yii::t('easyii', 'Clear image')?></a>
    <?php endif; ?>
...
<?php if (IS_ROOT) : ?>
    ...
    <?= $form->field($model, 'loan_type_id')->dropDownList(ArrayHelper::map(LoanType::find()->all(), 'id', 'name')) ?>
